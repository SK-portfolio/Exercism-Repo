...
namespace estate_executor{
    int assemble_account_number(int secret_modifier){
        int account_no{zhang::bank_number_part(secret_modifier) + khan::bank_number_part(secret_modifier) + garcia::bank_number_part(secret_modifier)};
        return account_no;
    }
    
    int assemble_code(){
        int totalR{zhang::red::code_fragment() + khan::red::code_fragment() + garcia::red::code_fragment()};
        int totalB{zhang::blue::code_fragment() + khan::blue::code_fragment() + garcia::blue::code_fragment()};
        return totalR * totalB;
    }
}
